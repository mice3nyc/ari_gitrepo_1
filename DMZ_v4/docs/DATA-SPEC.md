...
위치: `_dev/DMZ_v4/shared/photos_manifest.csv`
...
위치: `_dev/DMZ_v4/shared/dmz_blanks.csv`

컬럼:
```
story_id, blank_key, answer, alt_answers, in_source, answer_from, evidence, hint
```

- `story_id`: `s0102`
- `blank_key`: `A`, `B`, `C`, `D`, `A1`, `A2`, `C2` 등 가변
- `alt_answers`: 세미콜론(`;`) 구분
- `in_source`: 빈칸이 위치한 source ID
- `answer_from`: 정답을 찾을 수 있는 source ID

→ v3.2 CSV는 12 빈칸만. v4 갱신 시 18 스토리 → 약 70+ 빈칸 (cat04~06 추가 시 더 늘어남).
