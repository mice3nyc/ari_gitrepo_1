...
> 두 빌드(mobile / offline) 차이, fork 지점, 빌드 산출 방법, GitHub Pages URL 분기. 코드 작업 + 배포 시 핵심 참조.

## 두 빌드 비교

| 항목 | **mobile** | **offline** |
|------|-----------|-------------|
| 사용처 | 인턴 베타 데모 | 피스트레인 부스 (공간 설치물) |
| 일정 | 5/26 (월) 통일부 대회의실 24명 | 6/12~14 소이산 전망대 |
| 자료 표시 | 모든 자료 본문+빈칸 표시 | A/C는 표시, B/D는 가림 → unlock |
| 출력물 의존 | 없음 (모바일 단독 가능) | 필수 (B/D 본문은 출력물에서) |
| 토글 키 | `OFFLINE_MODE = false` | `OFFLINE_MODE = true` |
| 디바이스 | 모바일 웹 | 모바일 웹 + 공간 출력물 |
| URL | `.../DMZ_v4/mobile/` | `.../DMZ_v4/offline/` |
...
#!/bin/bash
# DMZ v4 두 빌드 산출
SHARED="_dev/DMZ_v4/shared/index_base.html"

# mobile: 그대로 카피
cp "$SHARED" "_dev/DMZ_v4/mobile/index.html"

# offline: OFFLINE_MODE 줄만 true로
sed 's/const OFFLINE_MODE = false;/const OFFLINE_MODE = true;/' "$SHARED" > "_dev/DMZ_v4/offline/index.html"

# photos 자산 심볼릭 링크 (또는 카피)
ln -sfn "../shared/photos" "_dev/DMZ_v4/mobile/photos"
ln -sfn "../shared/photos" "_dev/DMZ_v4/offline/photos"
```
...
│   └── index.html
└── DMZ_v4/
    ├── mobile/index.html       ← https://mice3nyc.github.io/ari_gitrepo_1/DMZ_v4/mobile/
    └── offline/index.html      ← https://mice3nyc.github.io/ari_gitrepo_1/DMZ_v4/offline/
```
...
- **localStorage 키 충돌**: mobile/offline이 같은 도메인(`mice3nyc.github.io`)이라 localStorage 공유. 빌드별로 키 prefix 구분 권장:
  - mobile: `dmz_v4_mobile_*`
  - offline: `dmz_v4_offline_*`
