...
function isUnlocked(storyId, sourceId) {
  const unlocks = JSON.parse(localStorage.getItem('dmz_v4_offline_unlocks') || '{}');
    ...
    if (OFFLINE_MODE) {
      ...
      if (sourceId === 'B' || sourceId === 'D') {
        ...
        if (!unlocks[currentStory.id].includes(sourceId)) {
          ...
          localStorage.setItem('dmz_v4_offline_unlocks', JSON.stringify(unlocks));
        }
...
상수로 빼서 관리:
```js
const LS_KEY = 'dmz_v4_state';                    // 또는 플레이어 이름별
const LS_TUTORIAL_KEY = 'dmz_v4_tutorial_done';
const LS_OFFLINE_UNLOCKS_KEY = 'dmz_v4_offline_unlocks';
```

플레이어 이름별 분리(인턴 24명 동시 플레이 환경 대응):
```js
const LS_KEY = `dmz_v4_state_${state.playerName || 'anon'}`;
```
...
```js
const LS_PREFIX = OFFLINE_MODE ? 'dmz_v4_o_' : 'dmz_v4_m_';
