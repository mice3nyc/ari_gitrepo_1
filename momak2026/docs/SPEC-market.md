...
#### 4.2 이벤트층 — GM 수동 충격 (spike/crash) — 2026-07-12

톱니 사이클 위에 얹는 방향성 충격. 지속시간 동안 해당 아이템의 톱니 모션을 대체한다(§4의 "아직 안 만든 층"을 구현).

- **폭등(spike)**: 스텝마다 `price += 폭`. 밴드 고점 초과 허용(급등). 지속 종료 → 밴드 상단으로 복귀(`high-1` 스냅) 후 톱니 재개.
- **폭락(crash)**: 스텝마다 `price -= 폭`. 저점 하회 허용, **바닥 1**까지. 지속 종료 → 밴드 하단(`low`)으로 복귀 후 톱니 재개.
- **폭(magnitude)**: GM 지정. 미지정(0/빈칸) → **랜덤**(그 아이템 파생 step의 2~5배 범위, 스텝마다 재추첨).
- **지속(durationSec)**: 초. 스텝 cadence = 그 아이템 `stepSec` 그대로, 타이머는 실초 감소.
- **동시성**: 아이템당 이벤트 1개(재발동 시 갱신). 여러 아이템 각각 독립. Market `OPEN`일 때만 진행(PAUSED면 정지).
- 스냅샷 아이템에 `event: 'spike'|'crash'|null` → 중앙화면 배지·색 강조.
- **명령**(WS `gm`): `itemEvent{itemId, kind:'spike'|'crash', magnitude?, durationSec}`.
...
- **구현(2026-07-12)**: 개별 아이템 low/high/변동시간/총주기 라이브 편집 = §4.1(중앙화면 `?edit=1` 오버레이). 충격 이벤트·셔플·채널섞기 = §10.1(GM 폰 컨트롤).

#### 10.1 GM 폰 컨트롤 화면 (`gm.html`, 모바일) — 2026-07-12

기존 마켓 제어(OPEN/PAUSE/RESUME/CLOSE/NEXT TURN) + 3종 추가. 폰 세로 화면 기준.

- **중앙 위치 셔플** `shuffleItems`: 중앙(overview) 16 아이템의 **표시 순서**를 랜덤 재배치. 서버가 `displayOrder` 셔플 → 전 화면 반영. 가격·세팅은 그대로, 자리만 바뀜.
- **채널 화면 섞기** `shuffleChannels`: 열린 채널 태블릿들이 보여주는 아이템을 **서로 순환 교체**. 서버가 채널 레지스트리(연결별 현재 items) 보유 → 아이템 배열을 채널 간 순환(랜덤 시프트, 전원 변경 보장) → 각 채널에 `setChannelItems` 푸시. 채널 0~1개면 무동작.
- **아이템 이벤트** `itemEvent`(§4.2): 아이템 선택 + 폭(선택, 빈칸=랜덤) + 지속(초) → [폭등][폭락].
- 채널 등록: `channel.html`은 `role=channel`로 접속, 선택 변경 시 `channelItems{items}` 전송(서버 레지스트리 갱신). 서버 `setChannelItems` 수신 시 표시 갱신.
