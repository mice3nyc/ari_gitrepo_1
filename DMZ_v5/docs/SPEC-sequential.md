...
```js
const LS_PREFIX = 'dmz_v5_s_';
const LS_STATE_KEY = LS_PREFIX + 'state';
const LS_TUTORIAL_KEY = LS_PREFIX + 'tutorial';
```

mobile(`dmz_v5_m_*`), offline(`dmz_v5_o_*`)과 완전 분리. 한 도메인에서 세 빌드를 다 플레이해도 진행 상태 충돌 없음.
...
- [ ] LS 키 prefix `dmz_v5_s_` 확인 (다른 빌드와 분리)
