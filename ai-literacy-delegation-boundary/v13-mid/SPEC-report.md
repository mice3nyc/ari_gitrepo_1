...
1. **상단 요약 스트립** (`showFinalReport`, 타이틀 바 바로 아래 — 학습자 유형 박스 위): 총점 / 능력레벨 / 위임레벨 세 칸(`.report-topstats` + `.rts-box`). 카툰 토큰(border-w·shadow·bg-card).
   ...
   - 라벨 순서·명칭은 피터공 발화대로 "총점 / 능력레벨 / 위임레벨". HUD `FX_META.levelLabel`(위임레벨=time·능력레벨=energy)과 명칭 일치. texts override: `final_report.stat_total/stat_ability_level/stat_delegation_level`(fallback 한국어).
