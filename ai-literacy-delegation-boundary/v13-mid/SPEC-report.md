...
### 4d. R3.5 — 항로 v2 (다음 회기 선문후코로 확정. 방향만 기록)
- 마이크로 항로: 시나리오 행 안에 1차→2차→검토 세 점. tier2의 가로 위치 매핑 규칙은 시안 게이트로 (현재 후보: tier1 칸 안에서 tier2 위임 깊이만큼 오프셋)
- 만화 타임라인: 하단 report-comic 섹션 폐지 → 항로 행 옆에 해당 시나리오 컷 동행. 시안 2후보 = 대표 컷 1~2장 / 좌항로·우만화 2단. getCutImageFor·getCutCaptionFor 재사용
- 인쇄 길이·SVG 폭 재계산 필요. 등급·점수는 행 헤더로 흡수 검토
