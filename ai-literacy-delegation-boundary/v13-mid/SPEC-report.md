...
### 4j. R3.11 — 상단 요약 스트립 + 시나리오 행 등급·점수 [피터공 6/16 세션491]

§4h-3에서 뺐던 상단 4박스(총점·레벨·원 미터)를 가볍게 되살린다. 다만 옛 4박스(원 미터 포함)가 아니라 **세 칸 요약 스트립**.

1. **상단 요약 스트립** (`showFinalReport`, 타이틀 바 바로 아래 — 학습자 유형 박스 위): 총점 / 능력레벨 / 위임레벨 세 칸(`.report-topstats` + `.rts-box`). 카툰 토큰(border-w·shadow·bg-card).
   - 총점 = `gameState.totalScore`
   - 능력레벨 = `_abilityCardCount()` (도메인 카드 cleared 장수, HUD energy 레벨과 동일 출처)
   - 위임레벨 = `_delegationCardCount()` (인간중심+성장 카드 cleared 장수, HUD time 레벨과 동일 출처)
   - 라벨 명칭: "시나리오 총점 / 능력레벨 / 위임레벨". 능력·위임레벨은 HUD `FX_META.levelLabel`과 일치. texts override: `final_report.stat_total/stat_ability_level/stat_delegation_level`(fallback 한국어).
   - **r43c (피터공 6/16)**: 큰 숫자+줄바꿈+작은 라벨 → **"라벨 : 값" 한 줄**(`.rts-lbl`·`.rts-colon`·`.rts-num` baseline 정렬). 박스가 채워지고 의미가 읽힘. 박스 배경 `--bg-card`(흰) → **하늘색 `--acc-cyan`**.
2. **시나리오 행 등급·점수** (`_renderDelegationMap`, `.rt-scnname` 셀): 시나리오 타이틀(rowspan=4) 아래 여백에 그 시나리오의 **등급 배지 + 점수**. 셀은 이미 `vertical-align:top`이라 타이틀 아래 공간이 빔.
   - 등급 = `r.grade`, 점수 = `r.finalScore` (history 레코드 필드, replay에서도 보존).
   - 타이틀 `.rt-scn-title` + 아래 `.rt-scn-grade`(배지 `.rt-grade-badge` + `.rt-scn-score`). 값 없으면 해당 조각 생략.

## §6. 이름 개인화 (피터공 6/29, 요청 [[요청.26.0629.1341-이름수업코드입장]])

입장 게이트에서 받은 `gameState.playerName`을 리포트에 표시. 개인화. 빈 이름이면 전부 기존 문구 그대로(무이름 안전).

- **(1) 최종 리포트 노란 타이틀 바** (`showFinalReport`, 11-report.js ~L628): `title_bar`("AI 리터러시 성장 리포트") 앞에 `"○○○의 "` 접두. 이름 `_esc()` 처리(상단 정의).
- **(2) 시나리오 활동 리포트 헤더** (`showReport`, 11-report.js ~L293): `<h2>` `scenario_report.title`("활동 리포트") 앞에 `"○○○의 "` 접두. 이름은 입력 시점에 `<>&"` 제거되어 안전.
- **(3) PDF 저장 파일명** (`printReport`, 11-report.js ~L787): `window.print()` 직전 `document.title`을 `"○○○_AI리터러시_성장리포트"`로 바꿔 브라우저 "PDF로 저장" 기본 파일명에 반영, 인쇄 후 복원.
- **학습자 유형 박스는 제외** (피터공 6/29 선택). `extractReportData().name`(L755)은 이미 존재 — 서버 페이로드용, 무변.
