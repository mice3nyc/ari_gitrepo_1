...
### §6-3 구현 (2026-08-16, r42)

둘 다 들어갔다. 「별도 빌드로」라고 적었지만 **r42에 같이 실었다** — 라이브 r41에 있는 버그를 그대로 둔 채 verbose만 켜면, 쿼터가 찬 판은 두 파이프 어디에도 안 찍혀 «로그를 켰는데 그 판만 조용한» 상태가 된다(§6-1). 관찰을 켜는 빌드가 관찰 구멍을 남길 수는 없다.

- `08-event-log.js` — `_EVENT_LOG_MAX = 200` 링버퍼. `l.push` 뒤 `slice(l.length-MAX)`
- `07-storage.js` — `saveGame()`을 try/catch로 감싸고, 실패하면 `eventLogKey`를 지우고 1회 재시도. 재시도도 실패하면 콘솔에만 남긴다(게임 흐름은 안 막는다)
- 부팅 시 1회 정리는 **넣지 않았다** — 링버퍼가 상한을 이미 묶으므로 부팅 정리는 이득 없이 이전 학기 기록만 지운다.

### §6-4 관찰 켜짐 (2026-08-16, r42)

`verboseLog:true` + `logEvEndpoint` 실주소. **수신 route가 실서버에 생기고 e2e를 확인한 뒤에 올렸다**(§8-1). 끄려면 `verboseLog:false` 한 줄이면 되고, A 파이프는 이 값과 무관하게 돈다.

버전도 `r41` → **`r42`**(mid·elem). `/stats`가 `byVersion`으로 세기 때문에 번호를 안 올리면 옛 빌드와 새 빌드가 한 칸에 섞인다(8/13 r41 승격 때와 같은 이유).
