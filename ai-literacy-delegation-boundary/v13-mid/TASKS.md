...
**관통 명제 — 「완주율 12%」는 학생이 그만둔 비율이 아니라 완주 기록이 살아남은 비율이다.**
`done:true`는 최종 리포트를 열 때 PUT되는데, 그 뒤 탭을 닫으면 `_ltRecordLeave`(`08d:71`)가 완주 여부를 안 보고 `done:false`를 다시 PUT하고, 서버 키가 `pid` 단위라 같은 파일을 덮는다. **덮여도 `sc[]`는 온전하므로 판정을 플래그에서 데이터로 옮기면 지금 쌓인 것으로 진짜 숫자가 나온다.**

실측(8/25 라이브) — 153판 · 완주 18(12%) · 그런데 깔때기는 `122→102→100→96→97`로 거의 안 좁아진다.

- [x] 원인 규명 — 코드 경로 확정(`11-report.js:623` → `08d:71` → 키 덮어쓰기)
- [x] `cur` 14건이 버그가 아니라 설계임을 확인(시나리오 «안»에서만 찍힌다)
- [x] 무엇이 살아남고 무엇이 사라지는지 필드 단위 확정(`sc[]` 생존 / `end` 소실)
- [x] 선문 — `infra/SPEC-log-query.md` (DONE-0xx · VIZ-0xx · EV-0xx · VER-0xx)
- [x] **DONE-010~040** — `full`(sc 기반)·`flagged`(done 기반) 두 기준 동시 산출 + 계측 손실 하한 + 유효 판 정의
- [x] **VIZ-010~060** — 상단 카드 넷 · ★진짜 깔때기 · 멈춘 자리 두 층 · 재도전 궤적 · 소요 분포 · 기존 구획 보존
...
- [ ] 파이프 C 정합 — `08c`가 동현공 Lambda로 보내는 `clientId`(기기 영속 UUID)와 그쪽 CloudWatch의 IP·UA가 「수집항목-설명」에 없다. 교육청 배포라 미룰 문제가 아닐 수 있다
- [ ] nodejs22.x 상향 — 이 템플릿 Lambda 넷이 `nodejs20.x`(볼트 전체 35군데 중 넷). **9/2·9/4 지난 뒤.** 배포 실물 런타임은 동현공에게 확인
...
- [x] **`rpLevelUpBonusByLevel` 0으로** — `{2:10,3:15,4:20,5:25}` → `{2:0,3:0,4:0,5:0}`. `src/js/00-config.js:51`. 등급 리워드(S30/A20/B15/C10/D5)만 남김. 이유: 보이는 레벨업 연출은 6/16에 폐기됐는데 숨은 RP 레벨업 보너스가 그대로 돌아 리워드가 화면 설명 없이 +10~25씩 커지던 것 제거(meterMaxByLevel은 전 레벨 100/100 평평 → 숨은 레벨 시스템 사실상 무력화). 초등·중등 둘 다 동일 수정.
- [x] **S 등급 폐지 → A 통합** (DECISIONS §10.17) — "S도 A". `getGrade`가 A부터 시작(95점+도 A), 최고 등급 A. S=30 리워드는 최고 등급이 A가 되며 자연 제거. `_judgePattern`은 `(S+A)` 합산이라 영향 없음. S 라벨·무드·30 리워드 config 항목은 미사용 dead로 잔존. 초등·중등 둘 다 동일.
- [x] **빌드** — `python3 build.py` → `index.html` **999,935 bytes**. 산출물에 반영 확인.
- [x] **커밋·푸시** — 커밋 **dc0e433**(main), GitHub push 완료 → 개발 라이브(github.io) 반영 확인. (학교 배포본 builds/mid는 별개·동현공 미전달.)

### ◐ 2026-06-22 — 교사 검토(김미진) 피드백 반영: groupwork "지운다" 문구 (8번/회신13번)

김미진 8번(회신문서 13번): "모둠 자료 찾을 때 이해한 내용 지우는 게 이해 안 됨". groupwork B3R2가 좋은 검토 행동(이해 못한 부분 빼고 설명 가능한 것만 남김)인데 "지운다"가 삭제·포기로 읽히고, 정작 핵심("설명할 수 있는 것만 남긴다")이 라벨에 안 담겨 헷갈림. 선택지 라벨 + 그 결과 만화 캡션을 결 맞춰 완결.
- [x] **B3R2 선택지 라벨** — "내가 이해하지 못한 문장과 조사하지 않은 내용을 지운다" → "AI 대본에서 내가 설명할 수 있는 내용만 남기고 나머지는 뺀다". `data/scenarios.yaml` line 3550.
- [x] **R2 결과 만화 캡션 18곳** — "내가 이해하지 못한 내용은 지웠다" → "내가 설명할 수 있는 내용만 남겼다" (cartoonCaption5+caption5, R2 경로 전체). 라벨과 일관.
- [x] **빌드 3종** — index.html(999,689) + builds/mid(999,639) + builds/elem 재빌드. 옛 표현 잔존 0.
- [ ] **다음 회기로 넘긴 콘텐츠 문구 후보** (백도 스캔, [[요청.26.0622.2202-콘텐츠문구손질]]): "지운다" 나머지(selfintro C1R2 1598·eorinwangja C1R2 5504·groupwork B1R2 3544), 라벨↔뜻 충돌(elem bookreport R2 "겉을 한번 본다" 334·elem animaltalk C2 lesson 2116), 오탈자(elem historycheck 7549 따옴표), 어휘·형식 7건.

### ◐ 2026-06-22 — 교사 검토(김미진) 피드백 반영: 어린왕자 독후감 "읽음/안 읽음" 모순

1차 교사 검토 5인 중 김미진(중1) 검토서 9번 지적: "앞에서 책을 읽는다고 했는데 읽지 않은 장면이 있는 설정이 이해 안 됨". 도입부가 "전혀 안 읽음"으로 읽혀, 뒤 선택지의 "읽다가 밑줄 친 문장" 등 이미-읽음 전제와 충돌. 도입부를 "일부는 읽음"으로 바꿔 해소. (초등 v13-elem은 도입부에 "읽은 부분은 있다"가 이미 있어 무영향 — scenarios 파일교체 방식)
- [x] **eorinwangja 도입부(situation.text) 교체** — "아직 책을 제대로 읽지는 않았다 / 지금부터 책을 다 읽기에는" → "앞부분은 읽었지만 끝까지 다 읽지는 못했다 / 남은 부분을 다 읽기에는". `data/scenarios.yaml` line 3927, 1곳.
- [x] **컷 캡션 일괄** — "독후감을 써야 하지만 책을 충분히 읽지 못했다" → "...책을 끝까지 읽지 못했다". cartoonCaption1+caption1 합쳐 54곳. 인코딩 검증 통과(백스페이스 0·NFC).
- [ ] **빌드 반영** — yaml만 수정됨. builds/mid에 반영하려면 중등 빌드 필요. 중등은 현재 마감 대상 아니므로 다음 중등 빌드에 합류 예정(피터공 확인 대기).
- [ ] **5인 검토 피드백 항목별 대응 로그** — 수용/보류/거절 추적 노트 부재(백도 진단). 별도 제안 중.
...
재도전이 무조건 가능해지면서 회복력("다시 시도 가능")·도전력("재도전 성공") 카드의 의미 소멸 → 게임에서 제거. 카드 모델 24장 → 22장(인간중심 12 + 도메인 10). SPEC §14.2.
- [x] **지급 지점 제거** — `03-engine.js`(회복력 자동 지급 + `fin.growthCard` 지급), `09-render-scenario.js`(도전력 지급 `challengeAwardedNow`/`improved` 블록 + cut6 회복력·도전력 팝업 라벨). 잔재 0 확인.
...
- [x] 커밋·푸시 — **e3db427** (bb18663..e3db427, main). GitHub Pages 라이브 자동 반영.
...
추가 2건 (같은 세션, 피터공 라이브 지적):
- [x] **⑥ 비용 숫자 처음부터 핑크** — `.cost-simple .cost-num` 기본색 `var(--acc-pink-deep)`(#d63f7a). CDP: 적용 전 `rgb(217,58,117)` 확인
- [x] **⑦ 할인 카운터 끝 −1 중복 교정** — `_fxRunCount` 마지막 비트(dv=0)에서 disc를 `0`으로 내려 finalpop 한 번만. 옛날엔 −1 멈춘 뒤 finalpop −1로 두 번 보임(N=1 특히). CDP: N=1 `['0']`·N=3 `[-2,-1,0]` 중복 없음
- [x] 재빌드 994,129B
...
#### 8/13 세션862 — 진입 수업코드 복수 허용 + 부팅 배지 발주처 표기 제거 (SPEC-intro-crt «입장 게이트»)

> 피터공: "진입시 사용하는 '하이러닝'과 함께 '경기교육'도 추가. 띄어쓰기 이슈는 알아서 보정. 하이러닝이 더이상 사용하지 않는 명칭이니 표시되는 곳 확인." → 요청 노트 [[요청.26.0813.0954-AI리터러시진입코드]]

- [x] `CONFIG.classCode:'하이러닝'` → `classCodes:['경기교육','하이러닝']` (00-config.js). 구 명칭은 화면에서 빼되 **받아는 준다** — 기존 안내물을 들고 오는 학생 때문
- [x] `_normClassCode`(공백 전부 제거)·`_isValidClassCode`(목록 중 하나와 일치) 신설. **기준값도 같은 규칙으로 정규화** — 입력만 보정하면 목록에 공백이 섞이는 순간 조용히 막힌다
- [x] `gameState.classCode` 저장값을 CONFIG 고정값 → **통과한 입력값**으로. 어느 코드로 들어왔는지 구분된다(로그 전송 스키마엔 미포함이라 수집 항목 영향 0)
- [x] 부팅 배지 `title_screen.badge` "경기도 하이러닝 - AI 리터러시" → **"AI 리터러시"** (피터공 결정). texts.yaml + ui_texts.csv + 코드 폴백 문자열. 위 TASKS의 «피터공 확인 — 배지(경기도 하이러닝) 유지 여부»가 여기서 닫힘
- [x] 하니스 신설 `scripts/verify-entry-gate-cdp.mjs` (17케이스). **옛 빌드에 먼저 걸어 8건이 붉어지는 것을 확인한 뒤** 새 빌드에 걸었다 — 안 붉어지는 하니스는 통과해도 아무 말 안 한 것
- [x] 빌드 기록 — `builds/mid/index.html` 778,180 → **778,366 bytes** · `builds/elem/index.html`(v13-elem) 798,162 → **798,348 bytes** (둘 다 +186, 같은 변경이 양 트리에 동일 반영된 신호). 개발본·elem-dev 재빌드 포함
- [x] 검증 — 중등 배포본 17/17 · 초등 배포본 17/17 · 중등 개발본 17/17 PASS
- [ ] **피터공 확인분** — 실제 화면에서 두 코드 진입 + 부팅 배지 문구
- [ ] **재배포** — 동현공에게 나간 8/8 zip은 옛 코드다. KT 명칭 변경 대응 건과 함께 재전달 필요

- [x] **r41 재패키징 (8/13)** — 진입 코드 변경분을 동현공 전달본에 반영. 버전 `r40`→**`r41`**(00-config·build.py 변종 치환·HANDOFF 3곳). **버전을 올린 이유: `/stats`가 `byVersion`으로 세는데 같은 번호로 내보내면 옛 빌드와 새 빌드가 한 칸에 섞인다.**
...
### r42 재전달 — 라이브가 6/30자였다 · 2026-08-18

- [x] **라이브 실측 — 두 도메인 다 `r39`, `last-modified: 2026-06-30`.** 8/4(r40)·8/8(r40)·8/13(r41)·8/16(r42) **네 번의 전달이 모두 라이브에 반영되지 않았다.** 두 달치 수정이 학생 화면에 없다
- [x] **원인은 우리 쪽 절차에도 있다** — 전달할 때마다 zip은 검증했지만 라이브를 한 번도 조회하지 않고 「전달 완료」로 닫았다. `curl` 한 줄이면 8/8에 잡혔다. **앞으로 전달분은 「라이브 반영 확인」까지 가야 닫는다**
- [x] **검사가 참인지 먼저 확인** — 옛 r40 전달본에 같은 하니스를 걸어 8건이 붉어지는 것을 보고 나서 r42에 걸었다. 통과만 보고 판정하지 않았다
- [x] **소스 재빌드가 바이트 동일 재현** — `build.py --variant={mid,elem} --release` 결과 md5가 기존 빌드와 완전 일치. r42 산출물이 현행 소스에서 나온 것임이 확정. **번호는 안 올렸다** — 내용이 같고, 라이브에 r42가 없어 `/stats byVersion`에서 섞일 것도 없다
- [x] **HANDOFF-deploy.md 개정**(mid·elem 동일) — 문서 맨 위에 §「이번 재전달의 이유」 신설(라이브 실측 표 + 수업코드가 급한 이유) · **배포 후 자가 확인 `curl` 두 줄**(버전 문자열이 `r42`면 완료, `r39`면 캐시 무효화 미실행) · 2절 4항에 캐시 무효화가 빠지면 S3에 올려도 옛 게임이 나간다는 것을 명시
- [ ] **동현공 전달** — 피터공이 텔레그램 발송
- [ ] **배포되면 라이브에 하니스 재실행** — `verify-entry-gate-cdp.mjs https://goe-ai-md.nolgong.com/index.html` (초등도)

**빌드 기록 (2026-08-18, r42 재패키징 · 코드 변경 없음)**
- 산출물 md5 8/16과 동일 — `mid` **782,383 B** `ec120cc2…` / `elem` **802,366 B** `04f4f778…`
- 패키지 `~/Downloads/AI리터러시_동현공전달_260818_r42/` + `.zip` **282엔트리 5,416,959 B**. 비ASCII 경로 282개 전부 UTF-8 플래그 · CRC 검사 OK. 8/16 zip과의 차이는 **디렉토리 엔트리 7개뿐이고 파일 282개는 전부 동일**(빈 폴더 `elem/images/_unused_tier1result/` 포함 — 파일이 없어 제외됨)
- 패키징 스크립트를 파이썬으로 고정(macOS `zip`은 UTF-8 플래그를 안 단다). 검증은 스크립트 안에서 플래그 누락 0·CRC를 자체 판정
- **압축 해제본으로 최종 확인** — 중등 17/17 · 초등 17/17 PASS(`경기교육`·`하이러닝`·띄어쓰기 3종 통과, 오타·부분·덧붙임·로마자 차단, 배지 「AI 리터러시」, JS 예외 0)
