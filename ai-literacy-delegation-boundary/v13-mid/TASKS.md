...
**최종 업데이트**: 2026-06-11 세션467 (시나리오 나가기 버튼 + 디버그 버튼 클릭 복구, SPEC §17)

### §17 — 시나리오 나가기 + 디버그 복구 (6/11 피터공, 세션467)

- [x] §17.1 디버그 버튼 클릭 복구 — 테마 z-index:1 강등 제거 + z-index:1300 (카드 독이 우측 하단 클릭 가로채던 것). 위치는 기존 우측 하단 코너 유지
- [x] §17.2 나가기 버튼 — HUD 우측 끝 `#scenario-exit`, 확인 모달 → 롤백(자원 스냅샷·이번 판 카드·pending) → 시나리오 선택 화면. node 시뮬 6건 PASS
- [x] texts.yaml 키 추가(exit_scenario·exit_confirm) + ui_texts.csv 재추출(277항목) + 빌드(911,350B)
- [ ] 브라우저 확인 (피터공): 디버그 버튼 클릭·초기화 / 시나리오 중 나가기 → 선택 화면·자원 원복
