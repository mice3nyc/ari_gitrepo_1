...
- **노출 게이팅**: `_isLocalEnv()`(file:// 또는 hostname localhost/127.0.0.1/0.0.0.0/빈값)일 때만 `_initDevNav`가 `hidden` 해제. 배포(github.io)·KT 전달본에선 숨김.
...
본편 화면, 시나리오 데이터, 리포트, 밸런스. 인트로 연출만.
