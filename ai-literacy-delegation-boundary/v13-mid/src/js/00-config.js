...
// =====================================================
var CONFIG={
  // 입장 게이트 수업코드 (피터공 6/29, 26.0813 복수 허용) — 공백 제거 후 비교. 변종(mid/elem) 공통값.
  // '하이러닝'은 구 명칭이라 기존 안내물로 들어오는 학생을 위해 남긴다. 화면엔 노출하지 않는다.
  classCodes:['경기교육','하이러닝'],
  ...
  logEndpoint:'https://w0a7nvx7qd.execute-api.ap-northeast-2.amazonaws.com/log',
  // 놀공 자체 수집 API — 플레이 로그 전송(08d, SPEC-log-transmit §7-8). 위 08c logEndpoint와 다른 스택이다.
  // 변종(mid/elem) 공통값 — 버전 구분은 레코드의 v 필드가 한다. ⚠️ 아래 clientIdKey~scenarios 두 줄은
  // build.py VARIANT_CONFIG_REPLACEMENTS의 앵커라 사이에 줄을 끼우면 elem 빌드가 죽는다.
  logApiEndpoint:'https://1js1lu6g60.execute-api.ap-northeast-2.amazonaws.com/log',
  ...
  // 끄려면 verboseLog를 false로 두면 된다(둘 다 있어야 켜지는 fail-safe라 엔드포인트는 남겨도 무해).
  // A 파이프(08b/08d + /report)와는 무관하게 병렬로 돈다 — 여길 꺼도 A는 그대로 간다.
  verboseLog:true,
  ...
  rpRewardByGrade:{S:30,A:20,B:15,C:10,D:5},
  rpLevelUpBonusByLevel:{2:0,3:0,4:0,5:0}, // 6/23 OFF — 보이는 레벨업 폐기(6/16) 후 숨은 보너스만 남아 리워드가 설명 없이 커지던 것 제거. 등급 리워드만 남김
