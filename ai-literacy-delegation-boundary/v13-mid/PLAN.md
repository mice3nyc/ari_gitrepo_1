...
> [!important] 로직/구조 작업은 v14로 이관 (2026-06-24)
> **현행 진입점(빌드 체계·로직 통합) → `../v14/`** + SPEC `../v14/SPEC-build-system.md`
> v13은 **라이브 배포 현행**으로 유지한다 — 동현공 → KT 하이러닝 배포는 계속 v13에서. 데이터(초등 콘텐츠 등) 작업도 당분간 v13에서.
> v14 = 단일 마스터 통합(평행 트리 → 게임기 한 대 + 카트리지 둘). **병행 기간 규칙**: 데이터는 v13, 로직/구조는 v14. v14 전환 직전 data 폴더 최종 동기화.
> 배경: [[요청.26.0624.1540-v14빌드체계통합]]
...
> _dev/CLAUDE.md 규칙 8 — 빌드를 놀공 외부(동현공·오공 등)로 넘길 때, "뭐가 달라 보이는가"를 기술 용어 없이 남긴다. TASKS의 기술 기록과는 다른 층.

### r40 — 2026-08-08 동현공 전달 (빌드 자체는 8/4자)

**한 줄**: 게임 화면과 진행 방식은 **하나도 안 바뀌었다.** 아이들이 어떻게 플레이했는지를 놀공이 볼 수 있도록, 익명 기록을 보내는 기능만 뒤에 붙었다.

- **아이들에게 달라 보이는 것**: 없다. 화면·문구·시나리오·점수 규칙 그대로다.
- **뒤에서 새로 생긴 것**: 시나리오를 하나 마칠 때마다, 그리고 학기를 완주할 때, 그 판의 기록이 놀공 서버로 조용히 전송된다. 인터넷이 끊겨 있으면 브라우저에 쌓아뒀다가 다음에 다시 보낸다. 어느 경우에도 게임이 멈추거나 느려지지 않는다.
- **보내는 내용**: 어떤 선택을 했는지, 등급이 무엇이었는지, 얼마나 걸렸는지, 어디서 그만뒀는지. **이름·학교·위치·아이들이 쓴 글·IP는 보내지 않는다.** 교육청 제출용 설명서(`수집항목-설명.md`)를 같이 넣었다.
- **동현공이 할 일**: 폴더 두 개를 올리고 URL 두 개를 주는 것, 그리고 예전부터 있던 참여 로깅 환경변수 갱신(`HANDOFF-deploy.md` 3절). **새로 붙은 전송 기능 쪽은 설정할 것이 없다** — 배포 도메인이 이미 허용 목록에 들어 있다. 도메인이 바뀔 때만 알려주면 된다.
