  ...
  sc: [                     // 시나리오별 (플레이한 것만, 순서 = 플레이 순서)
    {
      ...
      dur: 212,             // r40 — 최종 시도에 걸린 초 (없으면 생략 = 구 세이브 이어하기)
      du: [180,212],        // r40 — 시도별 소요 초 (재도전 있을 때만)
      gs: ["D","B"]         // r40 — 시도별 등급 순서 (재도전 있을 때만)
    }
    // ...
  ],
  cur: {                    // r40 — 미완료 판의 마지막 위치 (done=false일 때만, 시나리오 진입 이력 있을 때만)
    id: "career",           // 어느 시나리오에서 멈췄나
    step: "tier2",          // tier1 | tier2 | review | result — 어느 단계까지 갔나
    dur: 96                 // 그 시나리오에 머문 초
  },
...
### §1-2 r40 확장 (2026-08-04) — 라이브 수업 계측

> 배경: 학교 라이브는 되감을 수 없다. 배포한 뒤에 "이건 안 찍었네"를 발견하면 그 학기는 그냥 없는 데이터가 된다. 그래서 **수집 스키마만은 단계적으로 가지 않는다**(피터공 8/4). 분석 화면은 나중에 만들어도 되지만(데이터는 S3에 남아 있으니까), **안 찍은 것은 나중에 만들 수 없다.**

세 가지를 더 찍는다. 전부 §0의 "개인정보 0"을 지킨다 — 기기·화면·위치·이름 아무것도 늘리지 않았다.

| 필드 | 무엇 | 왜 이게 없으면 안 되나 |
|---|---|---|
| `sc[].dur` · `du` | 시나리오별 소요 초 | **수업 한 차시에 몇 개가 들어가나**를 이것 없이는 못 센다. 교사에게 줄 운영 지침(1차시 = 2개? 3개?)이 전부 여기 걸린다. |
| `sc[].gs` | 시도별 등급 순서 | 기존 `rep`은 "몇 번 다시 했나"만 안다. **다시 해서 나아졌는가**(D→B)가 이 게임의 교육 효과 주장 자체다. 마지막 결과만 남기면 그 질문이 원천 봉쇄된다. |

⚠️ **`gs`·`du`의 출처는 `scenarioHistory`가 아니다.** `replayScenario`가 롤백하면서 그 시나리오의 history 엔트리를 **`splice`로 지운다**(10-event-handlers [1]). 그래서 history에는 언제나 마지막 시도만 남고, 거기서 시도 이력을 읽으려는 시도는 조용히 1개만 돌려준다(8/4 하니스가 이걸로 2건 FAIL을 냈다). 대신 지워지지 않는 **`gameState._scAttemptLog[scid]`**(09 `goCut6`의 push 직후 append)에서 읽는다. 게임이 이미 갖고 있던 `gameState.replay[scid].firstAttempt`는 첫 판만 보존해 3회 이상 시도를 못 담으므로 쓰지 않았다.
| `cur` | 미완료 판의 마지막 위치 | `sc`에는 **끝낸 시나리오만** 들어간다. 도중에 그만두면 그 시나리오는 레코드에서 통째로 사라져, **어디서 놓쳤는지가 안 보인다.** 이탈 지점은 콘텐츠 수정의 1순위 재료다. |

`cur` 기록 시점: outbox 갱신은 시나리오 종료 때만 일어나므로 그것만으로는 도중 이탈을 못 잡는다. 그래서 **`pagehide`·`visibilitychange(hidden)`에서 한 번 더 upsert + flush**한다(08d `_ltRecordLeave`). 태블릿 앱 전환·화면 잠금이 수업 중 실제 이탈 형태라 `visibilitychange`를 같이 건다. 같은 pid를 덮어쓰는 구조(§3)라 여러 번 찍혀도 레코드는 하나다. **시나리오에 한 번도 안 들어간 사람은 기록하지 않는다**(타이틀만 보고 나간 빈 레코드 방지).

⚠️ **버전 표시를 `r39` → `r40`으로 올렸다.** 레코드의 `v`가 해석 기준이라(§1), 스키마가 바뀌었는데 같은 버전이면 분석할 때 `dur` 있는 판과 없는 판을 구분할 수 없다. S3 prefix도 `raw/v1.3-mid-r40/`로 갈려서 "전송 붙인 뒤의 데이터"가 저절로 분리된다.
