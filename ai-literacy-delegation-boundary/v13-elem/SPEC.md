...
- **잔여 버그 fix (6/21, 백공 재확인)** — 6/18 filter fix는 **선택 화면 재도전**(이미 `goNextScenario`를 거쳐 `totalScore`에 가산됨)만 정상화했고, **완료 화면(cut6) 직후 [다시하기]** 경로는 총점이 여전히 음수로 남아 있었다(예: 첫 시나리오 62점 획득 직후 다시하기 → 총점 −62). **근본 원인 = 반영 타이밍 비대칭**: `scenarioHistory.push`(finalScore 기록, `09:1132`)·역량 흡수(`absorbPending`, `goCut6`에서 호출 `09:1223`)·EXP 가산(`09:1077`)은 모두 **cut6**에서 일어나지만, `totalScore += score`만 **`goNextScenario`(다음 버튼, `10:169`)**에서 미뤄 일어난다. cut6 완료 화면은 `totalScore`에 아직 점수가 안 더해진 상태(0)인데, `replayScenario`가 `scenarioHistory`의 `finalScore`를 무조건 차감(`10:74`)해 음수가 됐다. **해결**: `[1]` 롤백의 `totalScore` 차감을 **`gameState.clearedScenarios.indexOf(scid)>=0`일 때만**(= `goNextScenario`를 거쳐 실제 가산된 경우만) 수행. 완료 화면 직후 재도전은 cleared에 없어 차감하지 않으므로 총점이 보존된다. 역량·EXP·카드 롤백은 모두 cut6 기준이라 타이밍이 일치해 무변(카드 할인은 `_lockedCardCount`가 clearedScenarios의 카드만 세므로 재도전 시나리오 카드는 자동 제외, 다른 완료 시나리오 카드는 유지). CDP 재검증: 완료화면 직후·선택화면 양쪽.
- **성장카드(회복력·도전력) 폐지 (6/21, 피터공)** — **재도전이 무조건 가능**해지면서 회복력("다시 시도할 수 있는")·도전력("재도전 성공")의 의미가 사라져 두 카드를 게임에서 제거. 카드 모델 24장 → **22장**(인간중심 12 + 도메인 10), 성장 2종 삭제. 지급 지점만 제거: `03-engine.js`의 `fin.growthCard` 지급 + B이하 회복력 자동 지급, `09-render-scenario.js`의 도전력 지급(`challengeAwardedNow`/`improved` 블록) + cut6 회복력·도전력 팝업 라벨. `growthCards` 트랙 인프라(state·독·리포트·인벤토리·로그 표시 코드)는 **빈 배열로 자연 비활성**(표시 코드는 빈 배열 안전 처리, 회귀 위험 최소화). 따라서 `_delegationCardCount`의 growthCards 항도 0 기여 → 시간 할인은 인간중심 카드만. yaml의 `growthCard:회복력`·`reportCardSummary`·R3a(D=회복력) 규약은 코드가 더 이상 읽지 않아 무효(데이터 잔재는 무해). `replayState.bestScore`/`bestGrade`는 재도전 비교 표시용으로 유지.

- **자원토큰 무한 복사 fix (QA8, 7/1, 피터공)** — cut6 완료 시 등급별 자원토큰(RP)이 `awardRP`(`09:1124`)로 즉시 적립되고, [자원 분배] 팝업은 1.4초 지연 체인(`09` 끝의 `setTimeout(...,1400)`)으로 뜬다. **분배 팝업이 뜨기 전에 재도전을 누르면** 두 결함으로 자원이 무한 지급됐다: **(1)** `replayScenario`가 시간/에너지는 스냅샷으로 원복하면서 `rp.balance`는 회수하지 않아 딴 토큰이 다시 도전으로 넘어가고, **(2)** cut6 보상 체인이 취소 불가 raw `setTimeout`이라 다시 도전 화면 위에서 스테일 분배 팝업이 발화해 원복된(가득 찬) 자원에 토큰을 또 얹었다. 등급 무관(리플레이 도달 가능하면 성립 — cut6 버튼은 B/A/S, C/D는 `showReplaySuggest`→`replayScenario`). **해결**: (1) 체인 `setTimeout`을 `_cut6ChainTimer`에 저장 + `_cancelCut6Chain()` 헬퍼(`09` 상단), `replayScenario`·`goNextScenario` 진입부에서 취소 → 스테일 팝업 차단([다음 시나리오로]를 팝업 전에 눌러 다음 화면에서 발화하던 변종도 함께 막힘). (2) `replayScenario` `[3a]`에서 `rp.balance>0`이면 `history`에 `forfeited_on_replay` 기록 후 `0`으로 회수(자원 원복과 일관, 타이머 취소 늦는 레이스까지 이중 방어). 초등·중등 동일 패치. 자가검증: node --check 4파일 OK · build clean(elem 1,027,729 / mid 1,009,542 bytes) · 산출물 패치 반영. 라이브 확인=태블릿(오공/피터공).
- **재도전 후 나가기 → 순차 잠금 오작동 fix (QA9, 7/2, 피터공)** — §14.2 재도전이 진입 즉시 `clearedScenarios`에서 `scid`를 빼는데(`[1b]`), §17.2 나가기(`confirmExitDo`)는 이걸 되돌리지 않았다. 완료 안 하고 나가면 그 시나리오는 "완료됨" 기록이 빈 채로 남아 §14.5 순차 잠금("미완료 중 다음 카드만 열림")과 충돌 — 이미 끝냈던 시나리오인데 잠긴 카드(div, 클릭 불가)로 보였다. 구멍이 하나면 자기 자신이 "다음"으로 잡혀 PLAY로 보이지만, **두 번째 재도전-나가기가 겹치면 먼저 생긴 구멍보다 뒤쪽 시나리오는 "다음" 자격을 잃고 완전히 잠긴다**(순서상 유일한 "다음" 슬롯은 최초 구멍이 가져감). **해결 — 이중 방어**: (1) `getEverClearedScenarios(gs)`(`02` 신규) — `clearedScenarios` OR `replay[sid].played===true`(재도전으로도 되돌아가지 않는 영구 완료 기록)로 "한 번이라도 끝낸" 목록을 구해, `showStartScreen`의 `isCleared`/`nextId`/잠금 판정과 `startScenario`의 §14.5 우회 방어 가드를 전부 이 목록 기준으로 교체 — 구멍이 있어도 잠기지 않는다. (2) `confirmExitDo` `[2b]` — `replay[scid].played===true`인데 나가는 시점에 `clearedScenarios`에 없으면(=재도전 중 나가기) 그 자리에서 다시 push — 애초에 구멍이 남지 않게. 역량·자원 롤백(§14.2 재도전 롤백)은 기존 그대로 시나리오 단위 스코프 유지(다른 완료 시나리오분은 무변, 해당 시나리오분만 롤백) — 전체 초기화(§14.3)와는 별개. 초등·중등 동일 패치. 자가검증: node --check 6파일 OK · build clean(elem 1,029,252 / mid 1,011,065 bytes) · CDP 헤드리스로 구버전 로직 재현(연속 재도전-나가기 2회 → 세 번째 시나리오 LOCKED 재현 확인) 및 신버전 무결 검증(같은 시나리오, 잠김 0 · 전부 재도전 버튼 · 역량 롤백 스코프 확인, 예외 0). 라이브 확인 대기(태블릿).
...
##### 14.5 순차 진행 잠금 (피터공 6/11, 세션466)

- **규칙**: 기본은 시나리오 순서대로만 진행 — 미완료 중 **다음(isNext) 카드만 진입 가능**. 그 뒤 미완료 카드는 **잠김**(클릭 불가). 완료한 시나리오의 재도전은 순서 무관 항상 가능(§14.2 유지).
- **UI**: 잠긴 카드 = div(버튼 아님), 점선 테두리+흐림+회색 "잠김" 마크(`.sc-lock`, `start_screen.mark_locked`). PLAY 배지는 §14.4 그대로 다음 카드에만.
- **가드**: `startScenario(sid)` 진입부에서도 차단(미완료 && !isNext → return) — UI 우회(저장 복원·콘솔) 방어. replayScenario는 cleared 검사라 무변.
- **AC**: 새 게임에서 1번만 진입 가능, 2~5 잠김. 1 클리어 후 2 열림 + 1 재도전 가능. 전체 클리어 후엔 잠김 없음.
...
#### 17. 시나리오 나가기 버튼 + 디버그 버튼 클릭 복구 (세션467, 6/11)

##### 17.1 디버그 버튼 클릭 복구 + 코너 고정
...
- **동작**: 클릭 → 확인 모달(`#exit-confirm-modal`, reset-confirm 패턴 재사용) → 확인 시 `exitScenario()`:
  ...
  3. pending·선택 상태 초기화(`pending 0`, `_prevPending 0`, tier/selected/score/completed 리셋), `currentScenarioId=null`
  4. `railClear()` + `saveGame()` + `trackEvent('scenario_exited',{scenarioId})` → `showStartScreen()`
- **§14.5와의 관계**: 나간 시나리오는 클리어되지 않은 상태 그대로 — 순차 진행상 여전히 "다음 카드"라 재진입 가능. 카드를 제거했으므로 재진입 시 같은 카드 재획득은 중복이 아님(`_ownedThisScenario` 충돌 없음).
- **컷6에서 나가기**: 그 판 통째 버려짐(점수·완료 확정은 `goNextScenario`에서만 일어나므로 자연 롤백). 리플레이 중 나가면 이전 기록은 리플레이 시작 시점에 이미 롤백된 상태 — 시나리오 선택 화면의 재도전(§14.2)으로 다시 진입 가능. (QA9, 7/2) 단, 이 롤백이 `clearedScenarios`에서 `scid`도 빼놓은 채라 §14.5 순차 잠금과 충돌해 잠긴 것처럼 보이던 버그가 있었음 — `[2b]`에서 `replay[scid].played`면 다시 push해 원상복구. §14.2 QA9 항목 참조.
- **알려진 잔여**: `competencies.*.history`의 `pending:true` 로그 엔트리는 남음 — 값 계산에 쓰이지 않는 로그성 데이터(흡수는 카운터 기준).
- **텍스트**: `texts.yaml` `game_flow.buttons.exit_scenario`("나가기") + `ui_messages.exit_confirm.{title,subtitle,btn_cancel,btn_destructive}`. JS fallback 내장. ui_texts.csv 재추출 동기 필수.

#### 18. 자원토큰 보상 상향 (세션468, 6/11)

- **요청 (피터공)**: "시나리오 완료시 타임충전 보너스 수치를 조금 높이자. 지금은 3,4,5 뭐 이렇게 너무 낮은 느낌이다. 최소 두자리 수로 하자."
- **변경**: `CONFIG.rpRewardByGrade` {S:5, A:3, B:2, C:1, D:0} → **{S:30, A:20, B:15, C:10, D:5}** / `CONFIG.rpLevelUpBonusByLevel` {2:3, 3:4, 4:5, 5:6} → **{2:10, 3:15, 4:20, 5:25}**.
- **밸런스 근거**: 스테이지당 원시 비용 9~27 × multiplier 1.25 → 시나리오당 시간 소모 수십 단위. 기존 S:5는 한 스테이지 비용의 1/4도 못 메움 — 상향 후에도 회복 < 소모라 "회복 없는 학기" 긴장은 유지(use-it-or-lose-it 무변).
- **D도 보상 (2차, 피터공)**: 1차는 D:0 유지였으나 "D도 한자리라도 주자" → **D:5**. 유일한 한 자리 — 두 자리 등급들과의 격차가 곧 등급 피드백.
- **시스템 무변**: 분배 모달(§8.10a)·rpCost(1토큰=+1)·메터 max 클램프 모두 그대로. 수치만 교체.
