...
**최종 업데이트**: 2026-06-24 (v14 빌드/개발 체계 통합 — 단일 마스터화)

### ★ 2026-06-24 — v14 통합: 빌드/개발 체계 단일 마스터화
요청: [[요청.26.0624.1540-v14빌드체계통합]] · SPEC: `SPEC-build-system.md`

배경: v13-elem·v13-mid 평행 트리로 로직 이중 관리되던 것을 단일 마스터(mid 기반) + 데이터 오버레이로 통합. v13은 배포 현행 유지(동결 마킹).

통합:
- [x] v14 분기 (v13-mid 마스터 복제)
- [x] 시나리오 ID 목록 단일화 — 5곳 하드코딩 → `CONFIG.scenarios`: `02-state` _scNum 신설, `11-report`, `12-debug` order, `15-card` pilot 판정
- [x] 이미지 경로 `../images` 통일 (build.py가 변종 시 자기완결 치환 — `build.py:262`)
- [x] `15-card` awardCards 우선 분기(elem 상위호환) 채택 → 중등 fallback 자동
- [x] 버전값 v14 교체: storageKey 4개 + version + build.py 변종 앵커 + shell title/version-label
- [x] 초등 완본 데이터 이식 — `v13-elem/data` 완본 yaml 5종 → `data/elem` 오버레이 (평행 트리 stale 해소: TODO 3075→0, awardCards 0→60)

검증:
- [x] 4종 빌드 성공 + build.py 정합(micro/aiflags 전수 대조) 통과
- [x] debug 플래그: 배포 false / 개발 true 확증
- [x] 회귀 동일성 PASS — v14 초등 SCENARIOS == v13-elem **diff 0** (awardCards 60·텍스트·카드지급·이미지 매핑 동일). 백도 검증
- [ ] 피터공 눈 확인 — 중등/초등 개발 빌드 플레이

빌드 기록 (2026-06-24):
| 빌드 | bytes | md5 |
|---|---|---|
| 중등 개발(루트) | 1,000,795 | f0d748f38935 |
| 중등 배포 builds/mid | 1,000,745 | 5a5fa5c3c3ee |
| 초등 배포 builds/elem | 1,029,888 | ac2e8d0dbb44 |
| 초등 개발 builds/elem-dev | 1,029,887 | abcc2c88a6a6 |

남음: 되감기 QA 기능(별도 단계, SPEC §7) — v14 통합 확정 후.

---

## TASKS — v1.3-mid (중등) — v13 이월 기록 (동결, 참조용)
