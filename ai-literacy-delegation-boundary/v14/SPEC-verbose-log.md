...
**§6-1에서 실측한 값으로 갱신(2026-08-13)** — 한 판 = 이벤트 **58건**, `snap` 포함 **29.1KB**.

| | 판당 | 판당 PUT | 학기 총량 | 학기 비용 |
|---|---|---|---|---|
| A. 우리 레코드 | **918B**(실측) | ~6 | 9MB | ~$0.3 |
| B. verbose(`snap` 포함 시) | **29.1KB**(실측) | 10~20 | 291MB | ~$1.0 |
| B. verbose(`snap` 제거 = 채택안) | ~8KB(추정, 58건×~140B) | 10~20 | 80MB | ~$1.0 |
