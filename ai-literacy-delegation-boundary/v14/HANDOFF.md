## AI 리터러시 v1.2-mid (중등) — 개발자 핸드오프

**작성일**: 2026-05-17 (세션365 v11 분기)
**대상**: 동현공 (코드리뷰 + 런칭 점검)
**라이브 URL**: https://mice3nyc.github.io/ari_gitrepo_1/ai-literacy-delegation-boundary/v12-mid/
**리포**: https://github.com/mice3nyc/ari_gitrepo_1

> v1.1에서 분기. **이 빌드는 중등용 — 기존 v11 데이터(자기소개·모둠발표·어린왕자·진로·시험공부 5 시나리오) 그대로 유지.** 초등용은 별도 `v12-elem` 빌드로 분기. 분기 사유: 이다연 선생님(초등 고학년 현장) 1차 피드백 — 시나리오 소재·역량 카드 수·튜토리얼·어투를 초등 대상에 맞춰 다시 잡아야 함. 엔진 코드는 양쪽 공통. 향후 데이터(시나리오·역량 카드) 변경은 빌드별로 진행.
>
> 본 HANDOFF 본문은 v1.1 시점 그대로(다음 회기 v1.2-mid 데이터 안정화 후 갱신).
