...
// v14: 시나리오 번호는 CONFIG.scenarios 인덱스로 도출(단일 소스). 하드코딩 매핑 제거.
function _scNum(scenarioId){
  var i=(CONFIG&&CONFIG.scenarios)?CONFIG.scenarios.indexOf(scenarioId):-1;
  var num=(i<0?1:i+1);
  return (num<10?'0':'')+num;
}
function getCutImage(scenarioId,cutNum){
  var n=_scNum(scenarioId);
  var base='../images/s'+n;
