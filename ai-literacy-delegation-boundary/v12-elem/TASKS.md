...
**최종 업데이트**: 2026-05-17 (v11 → v12-elem 분기, 세션365)

> [!info] 이 문서는 v12-elem (초등 고학년) 빌드 안 — v1.1 작업 큐 계승 + v12-elem 데이터 정정 단계 신설
> v12 분기 결정은 상위 `DECISIONS.md` 5/17 §, 요청 노트 [[요청.26.0517.2107-AI리터러시초중등분기빌드]], 이다연 피드백 [[26.0517 AI 리터러시 — 이다연 선생님 1차 피드백 정리]] 참조.
>
> **v12-elem 빌드 결과 (분기 직후)**: 867,927 bytes / Git `97387fe` / 라이브 https://mice3nyc.github.io/ari_gitrepo_1/ai-literacy-delegation-boundary/v12-elem/

---

### v12-elem 데이터 정정 단계 (신설, 5/17)

#### Phase 0 — 분기 빌드 세팅 ✅

- [x] v12-elem 폴더 신설 (v11 사본)
- [x] 좌표 정정 (storageKey·version·title·label·HANDOFF/SPEC 헤더)
- [x] 빌드 + JS syntax + storageKey 정합 검증
- [x] Git 커밋·푸시 (`97387fe`)
- [x] 메타 5종 헤더 콜아웃 (PLAN/TASKS/DESIGN-REGISTRY/SCENARIO-GUIDELINES/PAPERLOGY-THEME-HANDOFF)

#### Phase 1 — 입력 자료 수신 + 매핑 (5/17 진행)

- [x] 덱스 시나리오 1차 초안 위치 확인 + 수신 — `Assets/incoming/AI리터러시/codex/elementary-handoff-v12/` 11종 패키지
- [x] 이다연 선생님 5 이슈 분석 → [[26.0517 AI 리터러시 — 이다연 선생님 1차 피드백 정리]]
- [x] 덱스 패키지 1차 분석 → [[26.0517 AI 리터러시 v12-elem — 덱스 패키지 1차 분석]] (11종 검토 + 코드 변경 12자리 + 결정 5건 + 명명 충돌 한 자리)
- [ ] 피터공 검토(내일) — 결정 5건 + 명명: (1) 5번 시나리오 class_presentation vs AI 탐정 (2) 도메인 카드 수 3 vs 5 (3) 인간중심 세부 태그 노출 vs 숨김 (4) 카드 할인 모달 vs 자동 (5) 최종 리포트 도감 vs 축소 + 명명 충돌 결정
- [ ] 결정 후 옵션 A/B/C 진입 (추천 A: 시나리오만 먼저 진입)

#### Phase 2 — 시나리오 데이터 정정

- [ ] `data/scenarios.yaml` 새 시나리오 5개 작성 (덱스 초안 + 이다연 피드백 반영)
- [ ] 시나리오 선택지 어투 초등 친화 톤으로 (예: "잠은 죽어서 자면 되니까..")
- [ ] AI 윤리 결 시나리오 안 메카닉 자리 검토 (위임 메카닉 그대로 쓸지 / 윤리 결 메카닉 추가할지)
- [ ] `data/cuts.yaml` 이미지 매핑 정정 (새 시나리오에 맞게)
- [ ] 빌드 + JS syntax + 시나리오 5 keys 정합

#### Phase 3 — 역량 카드 + 튜토리얼

- [ ] `data/competency_cards.csv` 역량 카드 수 축소 (현 19 → 초등용 목표 수 결정)
- [ ] 카드 미리 보여주기 UI (획득 전 카드 후보 노출) — SPEC 신설 필요
- [ ] 튜토리얼 이미지 동반 (시간·에너지 줄어드는 장면·자원 직접 넣는 장면)
- [ ] `data/texts.yaml` 어투 정정 (초등 친화)

#### Phase 4 — 메카닉 밸런스 재검토

- [ ] 모범 플레이 시 시간 부족 → 오개념 우려 검증 (플테)
- [ ] `data/scenario_choices.csv` time/energy cost 재조정
- [ ] 선택 후 "다음이 궁금하지 않은 느낌" 원인 진단 + 정정 자리

#### Phase 5 — SPEC 본문 갱신 + 빌드 마감

- [ ] v12-elem `SPEC.md` 본문 갱신 (헤더 안내만 있는 상태 → 실제 변경 반영)
- [ ] v12-elem `HANDOFF.md` 본문 갱신 (대상 추가, 변경점 정리)
- [ ] 빌드 + 라이브 확인 + 이다연 선생님 2차 검토 요청
...
**빌드**: 832,048 bytes (v1.0 시점, 현 v12-elem = 867,927)
