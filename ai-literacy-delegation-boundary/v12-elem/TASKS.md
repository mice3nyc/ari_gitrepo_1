...
#### Phase 1 — 입력 자료 수신 + 매핑 (다음 회기 즉시)

- [ ] 덱스 시나리오 1차 초안 위치 확인 + 수신
- [ ] 이다연 선생님 5 이슈 분석 → [[26.0517 AI 리터러시 — 이다연 선생님 1차 피드백 정리]] 노트에서 시작
- [ ] 덱스 초안이 이다연 5 이슈에 어떻게 응답했는지 매핑 (대응 자리 / 미응답 자리 / 추가 결정 자리)
- [ ] 시나리오 결별 분류 — 위임 결 vs AI 윤리 결 (몇 개를 어느 결로 갈라낼지 결정)
