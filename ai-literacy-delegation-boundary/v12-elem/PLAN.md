...
> [!info] 이 문서는 v12-elem (초등 고학년) 빌드 안 — v1.1 사본 (다음 회기 정정 예정)
> 본문은 v1.0/v1.1 시점 그대로. 초등 대상에 맞춘 시나리오·역량 카드·튜토리얼·어투 정정은 다음 회기 진입(이다연 선생님 1차 피드백 + 덱스 시나리오 1차 초안 반영). v12 분기 결정은 상위 `DECISIONS.md` 5/17 §, 작업 요청 노트 [[요청.26.0517.2107-AI리터러시초중등분기빌드]], 이다연 피드백 [[26.0517 AI 리터러시 — 이다연 선생님 1차 피드백 정리]] 참조. 중등용 빌드는 별도 `v12-mid/`.

### v12-elem 정정 방향 (다음 회기 진입 자료)

- **시나리오 소재**: 자기소개·독후감은 초등 고학년이 AI 찾는 상황 아님 → 덱스 시나리오 1차 초안 수신 후 교체. 21명 반 학생 설문 기반(숙제·모르는 단어·심심한 대화·좋아하는 주제 조사·시험공부·발표준비)
- **시나리오 결**: 위임 결로만 가지 말고 일부는 AI 윤리 결로(심심한 대화·친구 관계·개인정보) — 정예공 자료 기반
- **역량 카드**: 수 축소 + 미리 보여주기(획득 전 후보 노출)
- **튜토리얼**: 이미지 동반(시간·에너지 줄어드는 장면)
- **선택지 어투**: 초등 친화 톤(예: "잠은 죽어서 자면 되니까..")
- **메카닉 부작용 점검**: 모범 플레이 시 시간 부족 → 오개념 우려. 밸런스 재검토
