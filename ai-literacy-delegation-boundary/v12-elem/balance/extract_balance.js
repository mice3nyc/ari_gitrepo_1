#!/usr/bin/env node
/**
 * extract_balance.js — AI Literacy v0.6 발란스 CSV 추출기
 *
 * index.html 의 CONFIG / DELEGATION_DELTA / SCENARIOS 객체를 읽어
 * 4종 CSV 를 balance/ 폴더에 생성한다.
 *
 * v0.6 변경 (5/1):
 *  - choices.csv에 8개 delta 컬럼 추가 (t1/t2/review × 위/도 + total)
 *  - tier1/tier2/review 객체에 delta:{위,도} 필드 있으면 채움. 없으면 빈값
 *  - cardSlots → axisDelta parent rename (코드 영향 없음, 데이터 패스스루)
 *
...
const choiceHeaders = [
  ...
  'discount_time_위-3', 'discount_energy_도-3',
  // v0.6 — 단계별 delta (위/도). yaml에 delta:{위,도} 필드 있으면 채움
  't1_delta_위', 't1_delta_도',
  't2_delta_위', 't2_delta_도',
  'review_delta_위', 'review_delta_도',
  'total_delta_위', 'total_delta_도',
...
scenarioOrder.forEach(sid => {
  ...
  leaves.forEach(leaf => {
    ...
    // v0.6 — delta 필드 추출 (yaml에 있으면 값, 없으면 빈문자열)
    function dlt(obj, axis) {
      if (!obj || !obj.delta || obj.delta[axis] == null) return '';
      return obj.delta[axis];
    }
    const t1Wi = dlt(t1Entry, '위'), t1Do = dlt(t1Entry, '도');
    const t2Wi = dlt(t2Entry, '위'), t2Do = dlt(t2Entry, '도');
    const rvWi = dlt(reviewEntry, '위'), rvDo = dlt(reviewEntry, '도');
    function sumIf(...vals) {
      const nums = vals.filter(v => v !== '' && typeof v === 'number');
      if (nums.length === 0) return '';
      return nums.reduce((a, b) => a + b, 0);
    }
    const totalWi = sumIf(t1Wi, t2Wi, rvWi);
    const totalDo = sumIf(t1Do, t2Do, rvDo);
    choiceRows.push([
      ...
      t1Wi, t1Do, t2Wi, t2Do, rvWi, rvDo, totalWi, totalDo,
      peterNote,
    ]);
  });
});
