...
**최종 업데이트**: 2026-05-05 세션298 — 5 시나리오 수정본 통합 + UI·점수 정정 4건 (C tier 재배치 + reviewLabels + 컷2·컷5 desc 제거 + B1 varPoint 정정)
...
##### 단계 8 — stage floor 박음 (5/4 세션289 추가)
75/135 leaf의 0 비용 자리 제거. yaml resourceCosts는 leaf 총비용 두고 formula에 floor(시간 2/에너지 1) 박음. 코드 + build_csv.py 적용. SPEC §3.5 floor 단락 추가. 가장 싼 leaf 한정 stage 합 = leaf 총비용보다 최대 4t/2e overage(의도). 상세는 [[26.0504 v0.7 작업 계획]] §단계 8.

##### 단계 9 — tier1 발란스 재조정 (5/4 세션289 추가)
A 25/20·B 6/5·C 14/11 (비율 1:0.24:0.56). yaml 일괄 shift A+5/+4·B-4/-3·C-2/-1. 가장 비싼 A1R3=52/41, 가장 싼 B1R1=10/7 (5.2x 차이). 상세는 [[26.0504 v0.7 작업 계획]] §단계 9.

##### 단계 10 — 점수→등급 재설계 (5/4 세션290 추가)
1차 합산 모델 분포가 위로 압축(basePoint 40 바닥에 깔려 회피·표절도 50+). 학습 의도 — "학습적 이상에만 고점, 대충 위임은 저점" — 안 박힌 자리. 처방: ① threshold 상향(S:95·A:90·B:80·C:70·D:60) ② basePoint 일괄 20→10 ③ varPoint 음수 도입(회피/부정 -10·정점 25) ④ R2 20→18. SPEC §6.6.7 신설(threshold 표 + varPoint 분포 + 분포 시뮬 + 135 leaf sweep). git push `2c1d503`·`c1a068c`. 상세는 [[26.0504 v0.7 작업 계획]] §단계 10.
...
- **review.varPoint leaf별 변수**: 합산 모델 단순 분배(R1=10·R2=18·R3=30)로 v0.7 진입, 플테 후 변수 추가 여부 결정 (단계 10에서 R2 20→18)
- **캐논 도입 별도 자리**: README가 자연스럽지만 학생이 게임 안에서 못 봄. ending 자리 추가 결정
- **A 등급 분포 회복**: 단계 10 sweep 결과 카드 매칭 시 A 영역(90~94) 통째 S로 흡수(정점·정점·R3·bonus 5 = 95). 학습 의도(누적 학습 보상) 정합이지만 A 안 보이는 자리 — 피터공 csv 콘텐츠 리뷰 후 결정
- **(3) 역량 시각화 방향**: effective × 2 발견(세션298 일차 점검, line 1738~1746). raw +6=effective +12 정상 작동. 결정 3안: (a) 유지 / (b) raw 그대로 표시 / (c) cap 강화 — 피터공 손
- **자원비용 정합**: 새 텍스트 C tier 의미(AI 위임·외부 의존·회피)와 기존 resourceCosts 정합 — csv 검수에서 미세 조정. 특히 studyplan C3(미루기)·eorinwangja C3(책 안 읽음) 시간 비용 자리
- **A·B tier 30 leaf 미세 조정**: selfintro·eorinwangja B1 두 자리 우선 정정(세션298 2α). 나머지 28자리는 csv 검수에서 한 자리씩
...
- [[요청.26.0505.2200-v07시나리오27]] — 시나리오 수정본 통합 (세션298)
- [[26.0505 v0.7 시나리오 수정본 작업 계획]] — 작업 계획 (세션298)
