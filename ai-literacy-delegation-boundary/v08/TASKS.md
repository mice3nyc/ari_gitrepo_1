...
**최종 업데이트**: 2026-05-08 세션313 (13d 체크 + SPEC 전체 동기화 — v16~v19 + 자원 충전 + 성장 카드)
...
#### 현재 단계: 성장 카드 기능 재설계 (단계 13d, §13+§14)
...
학기 종합 리포트의 산만한 6단계 → 학생 중심 통합 구조로.

- [x] 13.1 상단 4박스 → 2박스 (총점 + 레벨만, 위/도 2칸 제거)
- [x] 13.2 위/도 유형 박스(compType+compText) 삭제
- [x] 13.3 학습자 유형 (typeName+typeText) → 성장 리포트 (a) 블록
- [x] 13.4 카드 분포 (top 카드 + 인간/도메인/성장 카운트) → 성장 리포트 (d) 블록
- [x] 13.5 선택 패턴 (tier1 A/B/C + 해석) → 성장 리포트 (e) 블록
- [x] 13.6 _reportNarrative 호출 제거 (함수 코드는 보존)
- [ ] 13.7 자원 충전 너프 — recoverBase 0.45, S80/A70/B60/C55/D45, 레벨업 풀충전 제거 ✅ (0c46f3a)
- [ ] **검수 13-A**: 리포트 화면에 "위임 판단력", "도메인 지식" 수치 0건
- [ ] **검수 13-B**: 학습자 유형 + 카드 분포 + 선택 패턴이 성장 리포트 안에 통합
- [ ] **검수 13-C**: 상단 박스 2칸만 (총점 + 레벨)

---

##### 단계 13b — 선택지 카드 시각 위계 조정 (§3.4)

선택 텍스트가 카드에서 1순위로 눈에 들어오고, 비용 정보는 보조로 한 단계 뒤.

- [x] 13b.1 SPEC §3.4 시각 위계 + 카드 레이아웃 재설계 기록
- [x] 13b.2 CSS — 카드 구조 flex-column 전환 (세로 바 → 인라인 검정 박스 + 선택 텍스트)
...
##### 단계 13d — 성장 카드 기능 재설계 (§13 + §14)

회복력/도전력을 기능 카드로 격상. 디자인 차별화 + 리플레이 연결.

**회복력 트랙**:
- [x] 13d.1 지급 조건 확대: D only → B 이하 (onReview growthCard 로직 변경)
- [x] 13d.2 CSS — 성장 카드 전용 스타일 (점선 테두리 + 심볼 ↺/↑ + 어두운 톤)
- [x] 13d.3 화면 중앙 특별 카드 UI (기존 카드 팝업과 다른 레이아웃)
- [x] 13d.4 "회복역량 사용해 시간 되돌리기" 버튼 → 리플레이 진입 연결
- [x] 13d.5 "다음 시나리오로" 버튼 (사용 안 하는 선택지)

**도전력 트랙**:
- [x] 13d.6 리플레이 완료 chain에 도전력 카드 팝업 추가 (기존 런타임 로직 유지 + 시각 확인)

**공통**:
- [x] 13d.7 DESIGN-REGISTRY 성장 카드 스타일 등록
- [ ] 13d.8 build + 브라우저 확인 (B/C/D 결과 + 리플레이 경로)
- [x] 13d.9 SPEC §13 + §14 동기화 — §6.3 chain [1.5] 추가, §13.5 "B 이하" 정정

---

##### 단계 14 — 3차 빌드 + 종합 재검수

- [ ] 14.1 build.py 빌드 → 바이트 변화 확인
- [ ] 14.2 종합 자체 검수 QC-1~QC-14 + 신규 QC-15 전항목 통과
- [ ] 14.3 git 커밋 + push
- [ ] 14.4 GitHub Pages 배포 확인
...
| QC-15 | 리포트 통합 구조 | 학기 종합 리포트에서 위/도 수치 미노출 + 성장 리포트 안에 유형/카드/선택 통합 | 확인 |
