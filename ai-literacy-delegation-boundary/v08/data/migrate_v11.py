#!/usr/bin/env python3
"""v11 CSV → scenarios.yaml 마이그레이션
v0.8 단계 2 — 데이터 마이그레이션

입력: 02-ai-literacy-v11-final-integrated-data.csv (v11 단일 진실)
대상: scenarios.yaml (기존 구조 유지 + v11 새 필드 추가)

변경:
  1. finals: score/grade → v11 최종점수/최종등급 교체 + 새 필드 (카드, 피드백, 리플레이)
  2. reviewSupplements: v11 검토 학생 나레티브로 교체
  3. results: v11 텍스트로 교체 (tier2 단위)
"""
import csv
import sys
...
CSV_PATH = DATA_DIR / "02-ai-literacy-v11-final-integrated-data.csv"
YAML_PATH = DATA_DIR / "scenarios.yaml"
BACKUP_PATH = DATA_DIR / "scenarios.yaml.before-v11-migration"
...
def verify(data, rows_by_scenario):
    ...
    if mismatches:
        ...
        print(f"All 135 leaves: score/grade match ✅")
