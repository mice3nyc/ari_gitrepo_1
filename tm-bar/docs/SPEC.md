...
#### 사본 드리프트 감지 (26.0812 신설)

복사 배포라 **같은 파일이 두 곳에 실물로 있다.** 정본(`tm-bar/swiftbar-plugins/`)과 배포본(PluginDirectory = 현재 `todoy-bar/swiftbar-plugins/`). 설계상 의도된 중복이지만, **어긋나도 아무도 모르는 것이 문제다.**

어긋나는 경로가 둘이고 증상이 정반대다.
- **정본을 고치고 `install.sh`를 안 돌렸다** → 메뉴바는 옛 코드로 돈다. 고쳤는데 안 바뀐다
- **배포본을 직접 고쳤다**(SwiftBar가 읽는 파일이라 손이 먼저 간다) → 메뉴바는 맞게 도는데 리포엔 그 수정이 없다. 다음 `install.sh`가 조용히 덮어 **작업이 사라진다**

**그래서 배포본이 스스로 정본과 대조해 어긋나면 드롭다운에 빨간 줄을 띄운다.** 3초마다 도는 렌더가 검사 지점이다 — 사람이 기억해야 하는 검사(`install.sh --check` 같은 것)는 놓치는 날이 온다(오늘 아침 todoy-bar 날짜 경계와 같은 판단).

- 판정 = `cmp -s <배포본> <정본>`. 내용 비교이므로 **mtime 차이로는 안 뜬다** — git 조작이 mtime을 흔들어도 오경보가 없다(7/31 15:03 언스테이지가 정본 mtime만 5시간 밀어 둔 상태에서도 내용은 같았다)
- **어긋날 때만 출력한다.** 같으면 한 줄도 안 낸다(spec-guard와 같은 규율: 조건 불충족이면 조용히)
- 어느 쪽이 새것인지는 **단정하지 않는다.** mtime을 양쪽 다 보여주고 판단은 사람에게 넘긴다 — 위 두 경로가 정반대 처방을 요구하므로 잘못 단정하면 작업을 지운다
- 드롭다운에 배치한다. 메뉴바 타이틀은 노치 대비 최소 폭이라 건드리지 않는다
- 정본이 없으면(경로 변경 등) 조용히 넘어간다. 검사가 도구를 못 죽인다
- ⚠️ **이 검사는 배포본에 있어야 작동한다.** 정본만 고치면 검사도 배포가 안 된 상태라 첫 배포는 손으로 `install.sh`를 한 번 돌려야 한다
...
## 11. 컨텍스트 사용량 표시 (26.0823 선문, 코드 미착수)

> 배경: [[요청.26.0823.1023-메이커에반5편검토]] C1 · [[부록.26.0823-메이커에반1편-데스크탑앱]]

### 11.1 왜

CLAUDE.md 컨텍스트 관리 규칙은 70%면 정리, 80%면 즉시 `/memento`다. 그런데 **우리는 그 숫자를 못 본다.** 지금은 피터공이 `/context`를 쳐서 알려주는 것에 의존하고, 창이 여섯이면 여섯 번 물어야 해서 사실상 안 본다. auto-compact는 성능이 주저앉는 구간인데 그 절벽이 언제 오는지를 모르는 채로 일한다.

메이커에반 3편(19:47)도 같은 것을 플러그인으로 띄워 두고 "책상이 꽉 차기 전에 알아서 비우게 되니 그 자체로 습관이 바뀐다"고 한다.

### 11.2 재료 — 이미 다 있다 (26.0823 실측)

- 훅 stdin에 `transcript_path`가 통째로 들어온다. `data/hook-probe.log:3`(7/8 probe)에 그대로 찍혀 있다
- **그런데 `tm-hook.sh`는 그 JSON을 받아놓고 `.message`만 읽고 버린다**(20행 `JSON="$(cat 2>/dev/null)"`)
- 그 jsonl 마지막 `usage`의 `input_tokens + cache_read_input_tokens + cache_creation_input_tokens`가 현재 컨텍스트 크기
- 8/23 실측 여섯 창: 129k / 220k / 216k / 159k / 197k. 꼬리 300KB만 읽는 방식으로 **여섯 창 0.18초**

### 11.3 데이터 모델 추가

`data/windows/{ID}.json`에 필드 하나.

```json
"transcript": "/Users/p.air15/.claude/projects/-Users-p-air15-Neo-Obsi-Sync/{uuid}.jsonl"
```

**저장된 `session` UUID는 이 용도로 못 쓴다.** clear마다 자식 UUID가 갈리고 레코드는 옛 값을 안고 있다(8/23 창D의 `session`이 어제 것이었다). 살아 있는 값은 훅이 주는 `transcript_path`뿐이다.

### 11.4 tm-hook.sh — 한 줄 추가

`UserPromptSubmit`(working) 분기에서 `.transcript_path`를 뽑아 `{ID}.json`의 `transcript`에 쓴다. 값이 비면 안 쓴다. 나머지 동작은 그대로.

### 11.5 플러그인 렌더

창 헤드 라인 뒤에 절대값을 붙인다. `D · 메이커에반 5편 검토 · 283k`

- **절대값이 주 표시, 퍼센트는 보조.** 이유는 §11.6
- `transcript`가 없거나 파일이 없으면 아무것도 안 붙인다(옛 레코드 호환)
- 3초 갱신 × 여섯 파일 = 0.18초가 분당 20회다. **`transcript` 파일 mtime이 안 바뀐 창은 이전 값을 재사용한다**

### 11.6 함정 넷

1. **모델 한계가 transcript에 안 적힌다.** jsonl은 `"model":"claude-opus-5"`만 쓰고 `[1m]`을 안 쓴다. 파일만 봐서는 1M인지 200k인지 모른다. `~/.claude/settings.json`의 `"model": "opus[1m]"`을 읽어 기본값으로 삼되, 창에서 `/model`로 바꾸면 어긋난다. **그래서 퍼센트를 주 표시로 하면 잘못된 안심을 준다**
2. `/context`가 세는 값과 같지 않다. 시스템 프롬프트·툴·메모리를 버킷으로 안 가른다. 근사치로 쓴다
3. `"usage":{[^}]*}` 같은 grep은 **깨진다.** `output_tokens_details`가 중첩 중괄호라 첫 `}`에서 잘려 jq가 파싱에 실패한다(8/23에 실제로 `?`만 나왔다). 필드별로 따로 뽑는다
4. 정본과 배포본 두 벌 구조다. 고치면 `install.sh`까지 돌린다

### 11.7 덤 — 대화 수명

컨텍스트 값이 시간에 따라 로그에 쌓이면 「대화 하나가 망가지기 전까지 버티는 시간」은 별도 기능 없이 그 로그에서 나온다. 따로 만들지 않는다.

### 11.8 검증

| # | 검사 | 기대 |
|---|---|---|
| W1 | 수정 전 플러그인 출력에 토큰 표시 없음 | **없어야 정상**(붉음 확인) |
| W2 | `tm-hook.sh working`에 `transcript_path` 든 JSON 흘림 | `{ID}.json`에 `transcript` 생김 |
| W3 | `transcript_path` 없는 JSON | 필드 안 생김, exit 0 |
| W4 | 알려진 jsonl로 계산 | 이 창 283,056 등 손계산과 일치 |
| W5 | `transcript` 없는 옛 레코드 | 렌더 정상, 표시만 없음 |
| W6 | 파일 삭제된 경로 | 렌더 정상, 표시만 없음 |
| W7 | mtime 무변화 시 재계산 안 함 | 로그·타이밍으로 확인 |
| W8 | 여섯 창 렌더 실측 | 0.2초 이내 |

**실세션 확인**: 메뉴바에 실제 숫자가 뜨는지 피터공이 본다. 자가 시뮬 통과는 배선 확인이 아니다.
