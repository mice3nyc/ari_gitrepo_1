...
여러 클로드코드 창(A~F)이 "지금 뭘 하는지"를 메뉴바에 모으는 도구.
...
- ID는 A~F(6창). 창은 자기 ID를 기억 못 하므로 **진입 시 피터공에게 물어서** 정한다.
...
**잊어도 알려준다 (26.0812).** 복사 배포라 같은 파일이 두 곳에 실물로 있어서, 정본과 배포본이 어긋나면 **메뉴바 드롭다운에 빨간 줄**이 뜬다(양쪽 mtime + 처방 + 「차이 보기」). 두 방향 다 잡는다 — 정본 고치고 `install.sh`를 안 돌렸거나(고쳤는데 안 바뀜), **배포본을 직접 고쳤거나**(다음 `install.sh`가 조용히 덮어 작업이 사라진다). 어느 쪽이 새것인지는 단정하지 않으니 mtime을 보고 판단한다. 상세는 SPEC § 사본 드리프트 감지.
